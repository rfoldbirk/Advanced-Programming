...
public class PlayerView extends Tab implements ViewObserver {
    ...
    public void updateView(Subject subject) {
        if (subject == player.board) {
            ...
            statusLabel.setText("Checkpoints: " + player.getCheckpoint());
            for (int i = 0; i < Player.NO_REGISTERS; i++) {
            ...
            if (player.board.getPhase() != Phase.PLAYER_INTERACTION) {
                ...
                if (player.board.getCurrentPlayer() == player) {
                    ...
                    // Hent nuværende kort for at tjekke om det er aktiveret
                    // getStep -> hvilket register vi er på (kort der skal til at udføres)
                    // getProgramField -> tjekker programmet på den position vi er nået til
                    for (Command option : player.getProgramField(player.board.getStep()).getCard().command.getOptions()){
                        final Command finalOption = option;

                        Button optionButton = new Button(option.displayName);
                        optionButton.setOnAction(e -> gameController.executeCommandOption(finalOption));
