...
public class BoardFactory {
    ...
    // Skab et default boardname fra start af
    ...
    // Vi opretter board navnene her
    private final static String SIMPLE_BOARD_NAME = "simple";
    private final static String ADVANCED_BOARD_NAME = "advanced";

    // Liste af available boardnames
    private final List<String> boardNames = List.of(
            SIMPLE_BOARD_NAME, ADVANCED_BOARD_NAME
    );
    ...
    public Board createBoard(String name) {
        ...
        // Vi kan nu kalde de boards vi oprettede øverst i dokumentet
        if (boardNames.get(0).equals(name)) {
            return createSimpleBoard();
        } else if (boardNames.get(1).equals(name)) {
            return createAdvancedBoard();
        } else {
            return new Board(DEFAULT_WIDTH, DEFAULT_HEIGHT, DEFAULT_NAME);
        }
    }

    // jeg har lavet 2 boards
    // Første er det si mple
    public Board createSimpleBoard(){
        Board board = new Board(8, 8, SIMPLE_BOARD_NAME);
    ...
    public Board createAdvancedBoard(){
        Board board = new Board(8, 8, ADVANCED_BOARD_NAME);
        // add some walls, actions and checkpoints to some spaces
        Space space = board.getSpace(0,0);
        space.getWalls().add(Heading.SOUTH);
        ConveyorBelt action  = new ConveyorBelt();
    ...
    // listen henter listen af boards navne vi oprettede øverst i dokumentet
